...
func TestSequenceChangeComment(t *testing.T) {
	r := require.New(t)
	s := Sequence("test_sequence", "test_db", "test_schema")
	r.Equal(s.ChangeComment("new sequence comment"), `ALTER SEQUENCE "test_db"."test_schema"."test_sequence" SET COMMENT = 'new sequence comment'`)
}

func TestSequenceRemoveComment(t *testing.T) {
	r := require.New(t)
	s := Sequence("test_sequence", "test_db", "test_schema")
	r.Equal(`ALTER SEQUENCE "test_db"."test_schema"."test_sequence" UNSET COMMENT`, s.RemoveComment())
}

func TestSequenceRename(t *testing.T) {
	r := require.New(t)
	s := Sequence("test_sequence", "test_db", "test_schema")
	r.Equal(`ALTER SEQUENCE "test_db"."test_schema"."test_sequence" RENAME TO "test_db"."test_schema"."new_name"`, s.Rename("new_name"))
}
...
func TestSequenceIncrement(t *testing.T) {
	r := require.New(t)
	s := Sequence("test_sequence", "test_db", "test_schema")
	r.Equal(`SELECT "test_db"."test_schema"."test_sequence".nextval`, s.Increment())
}
